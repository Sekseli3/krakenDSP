...
`artifacts/kraken_dsp_stage_walkthrough.mp4`. Each stage plays the complete
input WAV by default, so a 20-second DI makes a roughly 160-second walkthrough.
Use `--seconds-per-stage 5` only when you intentionally want a five-second
excerpt per stage. Its stage audio retains the real DSP levels: it is
deliberately **not** normalised or gain-staged for the video.
