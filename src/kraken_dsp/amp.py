"""The first, deliberately small, real-time amp signal chain.

This follows the first milestone in the README:

    high-pass -> pre-emphasis -> asymmetric tanh -> low-pass -> cabinet -> output

It is intentionally a prototype.  The filters and cabinet state persist between
blocks so it can be used from a live audio callback as well as offline tests.
"""
...
class Version1Amp:
    ...
    def __init__(
        ...
        zero_input = self.settings.drive * self.settings.bias
        zero_shape = self.settings.positive_shape if zero_input >= 0.0 else self.settings.negative_shape
        self._waveshaper_zero = np.tanh(zero_shape * zero_input)
    ...
    def process_block(self, input_samples: np.ndarray) -> np.ndarray:
        ...
        shaped_input = self.settings.drive * (x + self.settings.bias)
        shapes = np.where(
            shaped_input >= 0.0,
            self.settings.positive_shape,
            self.settings.negative_shape,
        )
        x = np.tanh(shapes * shaped_input) - self._waveshaper_zero
